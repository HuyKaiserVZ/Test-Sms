...
loc = requests.get("https://ipapi.com/ip_api.php?ip=" + ip, headers={"Referer": "https://ip-api.com/", "Content-Type":
'application/json; charset=utf-8', 'User-Agent': 'Mozilla/5.0 (Linux; Android 7.1.2; Redmi 4X) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/77.0.3865.92 Mobile Safari/537.36'}).json()['country_name'].lower()
...
def bannerstart():
    ...
    banner = '''\n\n            ███████╗███╗   ███╗███████╗
        ...
        ╚══════╦═══════════════════╦══════╝
 ╔═════════════╩═══[ Spam  Sms ]═══╩═════════════╗
 ║    ╔═╗╔╦╗╔═╗  -> info: https://quanghuwngg.gq ║
 ║    ╚═╗║║║╚═╗  -> Contact: 0335220289          ║
 ║    ╚═╝╩ ╩╚═╝  -> Tool By Nguyễn Quang Hưng    ║
 ║      [1] Spam Sms     ║    [2] Spam Sms Pro   ║
 ╚═══════════════════════════════════════════════╝'''
    print(Colorate.Vertical(Colors.DynamicMIX((Col.light_red, Col.light_blue)), Center.XCenter(banner)))
    player = input(Fore.LIGHTWHITE_EX + "Input: ")
    if "1" in player:
        time.sleep(1.5)
        sms()
    elif "2" in player:
        ...
        print(Fore.YELLOW + "Command: [ " + player + " ] Not Found!")
        time.sleep(2)
        bannerstart()
#########################################################################################################################################
def sms():
...
╚══════╝╚═╝     ╚═╝╚══════╝\n'''
            ...
            distance = "<==============================================>"
            print(Colorate.Vertical(Colors.DynamicMIX((Col.light_red, Col.light_gray)), Center.XCenter(distance)))
            print("")
...
def smspro():
    def main():
        ...
        class SPAM:
            ...
            def banner(self):
                os.system("cls" if os.name == "nt" else "clear")
                title = '\n\n\n---Tool Spam Sms Pro By Nguyễn Quang Hưng---'
                banner = '''\n\n███████╗███╗   ███╗███████╗    ██████╗ ██████╗  ██████╗ 
██╔════╝████╗ ████║██╔════╝    ██╔══██╗██╔══██╗██╔═══██╗
███████╗██╔████╔██║███████╗    ██████╔╝██████╔╝██║   ██║
╚════██║██║╚██╔╝██║╚════██║    ██╔═══╝ ██╔══██╗██║   ██║
...
╚══════╝╚═╝     ╚═╝╚══════╝    ╚═╝     ╚═╝  ╚═╝ ╚═════╝\n'''
                ...
                ip1 = "Ip Của Bạn: " + ip
                loc1 = "Location: " + loc
                print(Center.XCenter(ip1))
                print(Center.XCenter(loc1))
                distance = "<==============================================>"
                print(Colorate.Vertical(Colors.DynamicMIX((Col.light_red, Col.light_gray)), Center.XCenter(distance)))
                print("")
            def random_string(self, length):
            ...
            def zalopay(self):
                try:
                    ...
                    if 'Thành công' in response:
                        print(self.format_print("*",f"ZALOPAY: {response}"))
                    else:
                        print(self.format_print("*",f"ZALOPAY: "+response))
                except:
                    print(self.format_print("*",f"ZALOPAY: SUCCESS!"))
...
███████╗╚██████╔╝╚██████╔╝██║██║ ╚████║
╚══════╝ ╚═════╝  ╚═════╝ ╚═╝╚═╝  ╚═══╝\n'''
        print(Colorate.Vertical(Colors.DynamicMIX((Col.light_red, Col.light_blue)), Center.XCenter(login)))

    def login():
        username = input("Tài Khoản: ")
        password = input("Mật Khẩu: ")

        if username == "nguyenquanghung":
            if password == "14102008":
                slowprint(Fore.LIGHTGREEN_EX + "Đăng Nhập Thành Công")
                time.sleep(1)
                main()
            else:
                slowprint(Fore.LIGHTRED_EX + "Tài khoản hoặc mật khẩu bạn đã nhập không chính xác")
                time.sleep(1)
                os.system("cls" if os.name == "nt" else "clear")
                banner0()
                login()
